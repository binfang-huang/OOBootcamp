...
import com.example.OOBootcamp.parkingstrategy.SequentialParkingOrderStrategy
...
class GraduateParkingBoyTest {
    ...
    internal fun should_park_in_order_and_get_ticket_when_park_given_parkingLots_is_not_full() {
        ...
        val parkingBoy = ParkingBoy(listOf(parkingLot1, parkingLot2), SequentialParkingOrderStrategy())
        val car = Car("川A44444")
    ...
    internal fun should_get_car_when_pickup_given_valid_ticket() {
        val parkingBoy = ParkingBoy(listOf(ParkingLot(1)), SequentialParkingOrderStrategy())
        val car = Car("川A55555")
        val ticket = parkingBoy.park(car)
    ...
    internal fun should_throw_InvalidTicketException_when_pickup_given_invalid_ticket() {
        val parkingBoy = ParkingBoy(listOf(ParkingLot(1), ParkingLot(1)), SequentialParkingOrderStrategy())
        val car = Car("川A55555")
        parkingBoy.park(car)
